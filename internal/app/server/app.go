...
func Run(cfg *config.Config) {
	...
	jwtManager := jwtmanager.New(cfg.App.SecretKey, cfg.App.TokenExp) // for user endpoints
	cryptManager, err := crypt.New(cfg.App.SecretKey)                 // cryptManager to encrypt data: key-value
	if err != nil {
		l.Log.Error(err)
	}
	// Service initialization with DB:
	userService := service.NewUserService(
		pg.NewUserStorage(pgx, cfg), // pg.NewUserStorage(postgresDB) // inmemory.NewUserStorage()
		jwtManager,
	)
